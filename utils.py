...
class WorldModel(object):
  ...
  def __init__(self, step, config):
    ...
    self.heads['reward'] = MLP(1, **config.reward_head)
  ...
  def preprocess(self, obs):
    ...
    obs['reward'] = getattr(torch.nn.functional, self.config.clip_rewards)(obs['reward']).unsqueeze(2)
    if 'discount' in obs:
...
class ActorCritic(object):
  ...
  def __init__(self, config, step, num_actions):
    ...
    self.critic = MLP(1, **config.critic)
    if config.slow_target:
      self._target_critic = MLP(1, **config.critic)
  ...
  def target(self, feat, action, reward, disc):
    reward = reward.to(torch.float32)
    disc = disc.to(torch.float32)
    value = self._target_critic(feat).sample()
    target = lambda_return(
        reward[:-1], value[:-1], disc[:-1],
